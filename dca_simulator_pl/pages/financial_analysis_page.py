"""
Strona Analizy Instrumentu Finansowego
Kompleksowa analiza zwrotów, rozkładów, wskaźników technicznych i prognozowania.
"""
...
def render_financial_analysis_page():
    """Główna funkcja renderująca stronę Analizy Instrumentu Finansowego"""
    
    st.header("📈 Analiza Instrumentu Finansowego")
    
    # Kluczowe założenia
    with st.expander("📚 Założenia i Metodologia", expanded=False):
        st.markdown("""
        **Założenia statystyczne:**
        - Zwroty traktowane jako szereg czasowy (mogą wykazywać autokorelację)
        - Rozkłady cen testowane na normalność vs grube ogony
        - Model GARCH przechwytuje grupowanie zmienności (volatility clustering)
        - Symulacje Monte Carlo wykorzystują parametry dopasowanego rozkładu
        
        **Analiza techniczna:**
        - Wstęgi Bollingera: 20-dniowa Średnia ruchoma ± 2 odchylenia standardowe
        - MACD: Wykładnicze średnie ruchome 12/26/9
        - RSI: Okres 14-dniowy (wyprzedanie <30, wykupienie >70)
        - Wskaźniki są opóźnione (oparte na danych historycznych)
        
        **Metody prognozowania:**
        1. Prophet: Model addytywny z dekompozycją trendu i sezonowości
        2. ARIMA: Zintegrowana średnia ruchoma z autoregresją (automatyczny dobór)
        3. Holt-Winters: Wygładzanie wykładnicze z trendem i sezonowością
        4. Monte Carlo: Symulacja stochastyczna z dopasowanym rozkładem
        
        **Ważne uwagi:**
        - Wyniki historyczne nie gwarantują przyszłych rezultatów
        - Wszystkie prognozy są probabilistyczne, nie deterministyczne
        - Używaj wielu metod do oceny niepewności prognozy
        - Nie uwzględnia kosztów transakcyjnych ani poślizgu cenowego
        """)
    
    st.markdown("""
    Kompleksowa analiza pojedynczego aktywa obejmująca testy statystyczne, wskaźniki techniczne
    i różne metody prognozowania.
    """)

    # Parametry użytkownika
    col1, col2 = st.columns(2)
    with col1:
        ticker = st.text_input("Symbol giełdowy (ticker)", "AAPL", help="Symbol papieru wartościowego").upper()
        start_date = st.date_input("Data początkowa", value=pd.to_datetime("2018-01-01"))
        end_date = st.date_input("Data końcowa", value=pd.to_datetime("today"))
        forecast_days = st.number_input("Horyzont prognozy (dni)", value=30, min_value=1, max_value=365,
                                       help="Liczba dni do prognozowania")
    with col2:
        mc_simulations = st.number_input("Liczba symulacji Monte Carlo", value=1000, min_value=100, max_value=10000,
                                        help="Liczba ścieżek symulacyjnych")
        mc_period = st.number_input("Okres prognozy (dni)", value=30, min_value=1, max_value=365,
                                   help="Horyzont prognozy Monte Carlo")
        mc_distribution = st.selectbox("Rozkład", ["normalny", "t-studenta"], index=0,
                                      help="Założenie o rozkładzie zwrotów")
        var_confidence = st.slider("Poziom ufności VaR", min_value=0.90, max_value=0.99, value=0.95, step=0.01,
                                  help="Poziom ufności dla Value at Risk")

    if st.button("Przeprowadź Analizę", type="primary"):
        # Pobieranie danych
        ...
        if data.empty:
            st.error(f"Brak danych dla {ticker} w okresie od {start_date_str} do {end_date_str}")
        else:
            # Compute returns
...
def _render_returns_analysis(ticker, data, returns):
    """Wyświetlenie szeregu czasowego zwrotów i statystyk opisowych"""
    st.subheader("📉 Szereg Czasowy Zwrotów Dziennych")
    fig_returns = px.line(
        returns,
        title=f"{ticker} - Zwroty dzienne",
        labels={'value': 'Zwrot dzienny', 'index': 'Data'}
    )
    ...
    stats_data = {
        "Statystyka": ["Średnia zwrotów dziennych", "Mediana zwrotów dziennych", "Zmienność dzienna (σ)", 
                     "Q1 zwrotów dziennych", "Q3 zwrotów dziennych"],
        "Wartość": [f"{returns.mean():.2%}", f"{returns.median():.2%}", f"{returns.std():.2%}", 
...
def _render_price_distribution(ticker, data):
    """Analiza i wyświetlenie historycznego rozkładu cen"""
    st.subheader("📊 Historyczny Rozkład Cen")

    # Obliczanie percentyla bieżącej ceny
    ...
    # Dodanie linii dla bieżącej ceny
    fig_hist.add_vline(
        x=current_price,
        line_dash="dash",
        line_color="red",
        annotation_text=f"Cena bieżąca: ${current_price:.2f}",
    ...
    st.markdown("### Analiza Percentylowa")
    st.markdown(f"""
    **Interpretation:**
...
def _render_returns_distribution(ticker, returns):
    ...
    st.subheader("📊 Analiza Rozkładu Zwrotów")
    ...
    st.markdown("### Analiza Dopasowania Rozkładu")
    st.markdown(f"""
...
def _render_unit_root_tests(data, returns):
    ...
    st.subheader("🔍 Testy Pierwiastka Jednostkowego (Analiza Stacjonarności)")
...
def _render_var_analysis(returns, var_confidence):
    ...
    st.subheader("⚠️ Wartość Narażona na Ryzyko (VaR) z Modelem GARCH")
    try:
...
def _render_monte_carlo(ticker, returns, mc_simulations, mc_period, mc_distribution):
    ...
    st.subheader("🎲 Prognoza Monte Carlo")
    ...
    if 'distribution_params' in mc_results and mc_results['distribution_params']:
        with st.expander("Parametry Rozkładu"):
...
def _render_technical_analysis(ticker, data):
    ...
    st.subheader("📈 Analiza Techniczna: Wstęgi Bollingera, MACD i RSI")
...
def _render_technical_summary(data, sma_bb, upper_band, lower_band, macd, signal, histogram, rsi):
    ...
    st.subheader("📊 Podsumowanie Analizy Technicznej")
    ...
    with col1:
        ...
        if bb_position > 80:
            st.error("🔴 BLISKO GÓRNEJ WSTĘGI - Wykupienie")
    ...
    st.subheader("📉 Ogólna Analiza Trendu")
    ...
    if bullish_signals >= 2:
        st.success("🚀 **TREND ZWYZKOWY** - Wiele wskaźników sugeruje wzrostową dynamikę")
        trend = "Bullish"
    elif bearish_signals >= 2:
        st.error("📉 **TREND SPADKOWY** - Wiele wskaźników sugeruje presję spadkową")
        trend = "Bearish"
    else:
        st.info("⚖️ **NEUTRALNY/MIESZANY** - Wskaźniki nie są zgrane")
        trend = "Neutral"
...
def _render_forecasting(ticker, data, forecast_days):
    ...
    # Holt-Winters Model
    st.subheader("❄️ Prognoza Holt-Winters")
    try:
...
def _render_prophet_components(forecast_prophet):
    ...
    st.subheader("🗓️ Komponenty Prophet (Trend i Sezonowość)")
