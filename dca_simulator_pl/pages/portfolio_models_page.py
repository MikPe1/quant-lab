...
def _render_basic_portfolio_section():
    ...
    if st.button("Oblicz CAPM i Metryki Portfela"):
        if not tickers:
            ...
            if market_data.empty:
                ...
                st.subheader("Wyniki Modelu CAPM")
                ...
                **Understanding the Sharpe Ratios:**
                ...
                if len(stock_returns) > 1:
                    st.subheader("Optymalizacja Portfela (Monte Carlo)")
                    
                    # Combine returns
                    returns_df = pd.DataFrame(stock_returns).dropna()
                    if returns_df.shape[1] < 2:
                        st.error("Potrzebne są co najmniej 2 akcje z danymi do optymalizacji portfela.")
                    else:
                        ...
                        st.subheader("Wagi Portfela o Maksymalnym Współczynniku Sharpe'a")
                        ...
                        # Copy-Paste Preview
                        with st.expander("Podgląd Formatu do Kopiowania", expanded=False):
                            st.caption("Copy this format for Portfolio Backtest page")
                            st.code(csv_string, language="csv")
                else:
                    st.info("Dodaj więcej tickerów do optymalizacji portfela.")


def _render_extended_analysis_section():
    """Render extended advanced portfolio analysis"""
    st.header("🎯 Zaawansowana Analiza i Optymalizacja Portfela")
    ...
    extended_tickers_input = st.text_area(
        ...
        help="Wprowadź 3-15 tickerów do analizy",
    ...
    with col3:
        if analysis_mode == "Backtesting (Validate Strategy)":
            ...
            st.info("Tryb Produkcyjny: Używanie 100% wybranego zakresu dat")
    ...
    if analysis_mode == "Production (Optimize for Future)":
        portfolio_method = st.selectbox(
            ...
            index=0,
            help="Auto-wybieranie wybiera metodę z najwyższym współczynnikiem Sharpe'a na pełnym zbiorze danych"
        )
    else:
        portfolio_method = st.selectbox(
            "Portfolio Construction Method",
            ["Auto-select (Best OOS Performance)", "HRP", "Equal Weight", "Inverse Volatility", 
             "Mean-Variance (Markowitz)", "Minimum Variance", "Risk Parity"],
            index=0,
            help="Auto-wybieranie wybiera metodę z najlepszym wynikiem kompozytowym poza próbą"
        )
    ...
    if st.button(button_text, type="primary"):
        if not extended_tickers or len(extended_tickers) < 3:
            st.error("Proszę wprowadzić co najmniej 3 tickery dla sensownej analizy portfela.")
        elif len(extended_tickers) > 15:
            st.error("Proszę użyć nie więcej niż 15 tickerów, aby uniknąć problemów obliczeniowych.")
        else:
            is_production = (analysis_mode == "Production (Optimize for Future)")
            with st.spinner("Przeprowadzanie kompleksowej analizy portfela... Może to potrwać kilka minut."):
                try:
                    ...
                    for i, ticker in enumerate(extended_tickers):
                        status_text.text(f"Pobieranie danych dla {ticker}...")
                    ...
                    if len(data) < 3:
                        ...
                        st.info(f"Pomyślnie załadowano dane dla {len(data)} tickerów: {', '.join(data.keys())}")
                        ...
                        if len(combined_data) < 504:
                            ...
                            if len(returns) < 500:
                                st.error("Niewystarczające dane zwrotów po przetworzeniu.")
                            else:
                                st.success(f"Dane przygotowane: {len(returns)} dni zwrotów dla {len(returns.columns)} aktywów")
                                
                                # Fetch S&P 500 benchmark data (full period for visualization)
                                st.info("Pobieranie danych benchmarku S&P 500...")
                                ...
                                if not sp500_data.empty and len(sp500_data) > 100:
                                    ...
                                    st.success("Dane benchmarku S&P 500 załadowane")
                                else:
                                    st.warning("Nie udało się załadować danych benchmarku S&P 500")
                                ...
                                if "Auto-select" in portfolio_method:
                                    if is_production:
                                        ...
                                        st.info(f"Auto-wybrana metoda dla produkcji: **{selected_method}** (Najwyższy Sharpe na pełnych danych)")
                                    else:
                                        # Backtesting: use OOS composite score
                                        selected_method = oos_results['best_method']
                                        st.success(f"Auto-wybrana metoda: **{selected_method}** (Najlepszy wynik kompozytowy OOS)")
                                else:
                                    ...
                                    if selected_method not in oos_results['all_weights']:
                                        ...
                                        st.info(f"Użytkownik wybrał metodę: **{selected_method}**")
                                
                                selected_weights = oos_results['all_weights'][selected_method]
                                
                                # Display results based on mode
                                if is_production:
                                    st.success("Portfel Produkcyjny Zoptymalizowany")
                                    st.markdown(f"""
                                    ...
                                    if selected_method == oos_results['best_method']:
                                        st.success(f"Analiza zakończona! Używanie najlepszej metody: **{selected_method}**")
                                    else:
                                        st.info(f"Analiza zakończona! Używanie wybranej metody: **{selected_method}** (Najlepsza była: {oos_results['best_method']})")
                    ...
                    st.info("Spróbuj z mniejszą liczbą tickerów lub innym zakresem dat.")
...
def _render_performance_summary_all(oos_results, selected_method, benchmark_metrics, is_production=False):
    ...
    if is_production:
        st.subheader("🎯 Metryki Portfela Produkcyjnego (Pełne Dane Historyczne)")
        st.caption("Te metryki pokazują historyczną wydajność używając wszystkich dostępnych danych. Monte Carlo pokazuje oczekiwane rezultaty przyszłe.")
    else:
        st.subheader("📊 Podsumowanie Wydajności (Poza Próbą) - Wszystkie Metody")
        st.caption("Te metryki pokazują prawdziwą wydajność poza próbą w celu walidacji strategii.")
...
def _render_portfolio_weights(selected_method, selected_weights, oos_results, returns, is_production=False):
    ...
    with col2:
        ...
        st.download_button(
            label="📥 Pobierz Wagi (Prosty Format)",
            data=csv_string,
...
def _render_complete_results_download(oos_results, selected_method, selected_weights, 
    ...
    st.subheader("📦 Eksport Pełnych Wyników")
    ...
    with col1:
        ...
        st.download_button(
            label="Pobierz Wydajność Wszystkich Metod",
    ...
    with col2:
        ...
        st.markdown("**Wybrany Portfel - Pełne Dane**")
        ...
        st.download_button(
            label="📦 Pobierz Pełne Dane Portfela",
            data=csv_complete,
...
def _render_cumulative_returns(oos_results, best_method, sp500_returns):
    ...
    st.subheader("Porównanie Zwrotów Skumulowanych")
...
def _render_stress_testing(oos_results, best_weights):
    ...
    st.subheader("Wyniki Testów Stresowych")
...
def _render_returns_distribution(oos_results, selected_weights):
    ...
    st.subheader("Rozkład Zwrotów Portfela (Poza Próbą)")
...
def _render_monte_carlo_forecast(returns, best_weights, mc_simulations, mc_period, mc_distribution):
    ...
    st.subheader(f"🎲 Prognoza Monte Carlo ({mc_period // 252} {'Lata' if mc_period > 504 else 'Rok' if mc_period > 252 else 'Rok'})")
...
def _render_recommendations(selected_method, oos_results, selected_weights, mc_results, is_production=False):
    ...
    st.subheader("💡 Rekomendacje Inwestycyjne")
